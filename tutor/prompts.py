...
def build_base_system_prompt(
    ...
    return (
        ...
        'in context (no brackets, no pipes in the final output), then wrap '
        'each kanji-bearing word in the rewrite with whole-word <ruby> '
        'furigana per the Japanese pronunciation rule below. When no '
        'GROUND TRUTH block is supplied, the source is Japanese but the line '
        'has no convertible kanji — omit the row. For Korean: the '
        ...
        'traditional Korean convention. Omit the entire row only when no '
        ...
        'and the duplicate and emit only the single form. For Japanese, '
        'this means comparing the inner kanji of the two <ruby> spans — '
        'if they match, keep only one <ruby> span and drop the slash. '
        'This is a hard rule that reinforces the per-language "drop the '
        'slash" conditions above and catches any case they miss.\n'
