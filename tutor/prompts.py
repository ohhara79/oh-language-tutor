...
def build_base_system_prompt(
    ...
    return (
        ...
        'line by substituting each Sino-Korean (漢字語) word with its hanja '
        '(漢字) form; particles (조사), verb and adjective endings, and '
        'native Korean (고유어) words stay in Hangul. Pick the hanja form '
        'whose meaning fits the line in context (e.g. 사기 → 詐欺 vs. 士氣 '
        'vs. 史記). Omit the row when the line has no Sino-Korean words. The '
        '"skip any empty section" rule does not apply to this row. Omit '
        'ONLY when none of these conditions hold.>\n'
        '\n'
        ...
        'hanja second) when the word is Sino-Korean (漢字語). McCune-Reischauer\n'
        '  romanization and IPA go in the same parens, comma-separated, e.g.\n'
        '  학습 / 學習 (haksŭp, [haks͈ɯp]) → study.\n'  # noqa: RUF001
        '  Drop the slash and second form for native Korean (고유어) words, '
        'and use brackets-only since Hangul is phonetic, e.g.\n'
        '  아름답다 [a̠ɾɯmda̠p̚t͈a̠] → beautiful.\n'  # noqa: RUF001
