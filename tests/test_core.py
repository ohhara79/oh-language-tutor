...
async def test_stdin_loop_appends_unexplained_entry():
    ...
    sink = _RecordingSink()
    await stdin_loop(sink, None, asyncio.Event(), input_file=src)
...
async def test_stdin_loop_filter_regex_skips_non_matching():
    ...
    sink = _RecordingSink()
    await stdin_loop(sink, re.compile(r'KOREAN'), asyncio.Event(), input_file=src)
...
async def test_stdin_loop_stop_event_breaks_loop():
    ...
    stop = asyncio.Event()
    stop.set()
    await stdin_loop(sink, None, stop, input_file=src)
